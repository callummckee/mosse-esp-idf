...
static camera_config_t get_camera_config() {
    ...
    config.ledc_timer = LEDC_TIMER_1; //servos use timer 0 as they have different clock frequency
...
void camera_task(void* pvParameters){
    ...
    while(1) {
        ...
        if(tracker->pendingInit) {
            tracker->initTracker(fb->buf);
        }
        if (config->newcfg) {
        ...
        if (tracker->isTracking) {
            ...
            float dt = (esp_timer_get_time() - start)/1000000.0f;
            float center_error_x = (tracker->target.x_pos + (tracker->target.true_width / 2.0f)) - (FRAME_WIDTH / 2.0f);
            float center_error_y = (tracker->target.y_pos + (tracker->target.true_height / 2.0f)) - (FRAME_HEIGHT / 2.0f);
            turret->move(center_error_x, dt, turret->pan_channel);
            turret->move(center_error_y, dt, turret->tilt_channel);
        }
        else {
            server->update_frame(fb);
            if (!server->pp.reader_busy) {
...
extern "C" void app_main(void) {
    ...
    Server* server = new Server(NULL, config);
    ...
    Tracker* tracker = new Tracker(turret);
